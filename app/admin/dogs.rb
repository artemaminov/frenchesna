ActiveAdmin.register Dog do
  ...
  permit_params :birthdate, :gender, :puppy, :rip, :mother_id, :father_id, :breed_id, :genealogy_link, :litter_id, avatar_attributes: [:id, :file, :_destroy], background_attributes: [:id], gallery_pictures_attributes: [:id, :file, :rcrop, :order, :_destroy], pictures_attributes: [:id, :order, :_destroy], litter_attributes: [:id, :title, :_destroy], translations_attributes: [:id, :locale, :fullname, :nickname, :about, :awards, :_destroy]
  ...
  form do |f|
    ...
    f.inputs 'Изображения' do
      ...
      panel "Управление изображениями", class: 'dragndrop' do
        ...
        f.input :pictures, as: :file, input_html: { multiple: true, direct_upload: true, name: 'dog[gallery_pictures_attributes][][file]' }
      end
    end
    f.actions
